...
async function fetchJson<T>(
	...
	try {
		...
		if (!res.ok) {
			...
			const safeMessage = sanitized.replace(
				/\/[^\s:]+\.(rs|js|ts):\d+/g,
				"[internal]",
			);
...
export const getTimingWindows = () =>
	fetchJson<TimingResponse>(`${BASE}/timing-windows`);
...
// Soil Tests
export const getSoilTests = () => fetchJson<SoilTest[]>(`${BASE}/soil-tests`);
...
export const getSeasonalPlan = (year?: number) => {
	const params = year ? `?year=${year}` : "";
	return fetchJson<SeasonalPlan>(
		`${BASE}/seasonal-plan${params}`,
		undefined,
		30_000,
	);
};
