	...
	return (
		<div>
			...
			<p style={styles.intro}>
				Track shrubs, bushes, and other plants alongside your turf. We'll
				generate a homeowner-level maintenance plan (pruning windows,
				fertilizing timing, etc.) and surface those windows in your Calendar,
				Seasonal Plan, and Recommendations.
			</p>

			{showForm && (
				<AddPlantForm onCreated={handleCreated} onError={setError} />
			)}
		...
		<div style={styles.row}>
			...
			{expanded && (
				<div style={styles.rowBody}>
					...
					<h3 style={styles.tasksHeading}>
						Maintenance tasks ({plan.tasks.length})
					</h3>
					{plan.tasks.length === 0 ? (
						...
						<ul style={styles.taskList}>
							{plan.tasks.map((task) => (
								<TaskRow
									key={`${task.task_type}-${task.window_start_month_day}-${task.window_end_month_day}`}
									task={task}
								/>
							))}
						</ul>
					...
					<div style={styles.rowFooter}>
						<div style={styles.footerMeta}>
							Plan generated {formatDate(plant.plan_generated_at)} ·{" "}
							{plant.plan_model}
						</div>
...
function TaskRow({ task }: { task: MaintenanceTask }) {
	return (
		<li style={styles.taskRow}>
			...
			{task.zone_note && (
				<div style={styles.taskZoneNote}>{task.zone_note}</div>
			)}
		</li>
	);
}
		...
		<form onSubmit={handleSubmit} style={styles.form}>
			<div style={styles.formGrid}>
				<div style={{ gridColumn: "1 / -1" }}>
					<label style={styles.formLabel} htmlFor="plant-name">
				...
				<div>
					<label style={styles.formLabel} htmlFor="plant-type">
						Plant type
					</label>
					<select
						id="plant-type"
				...
				<div>
					<label style={styles.formLabel} htmlFor="plant-location">
						Location (optional)
					</label>
					<input
						id="plant-location"
				...
				<div>
					<label style={styles.formLabel} htmlFor="plant-planting-date">
				...
				<div style={{ gridColumn: "1 / -1" }}>
					<label style={styles.formLabel} htmlFor="plant-notes">
						Notes (optional)
					</label>
					<input
						id="plant-notes"
