...
export default function Environmental() {
	...
	const depthRows = [
		{
			depth: "5 cm",
			temp: current?.soil_temp_5_f,
			moisture: current?.soil_moisture_5,
		},
		{
			depth: "10 cm",
			temp: current?.soil_temp_10_f,
			moisture: current?.soil_moisture_10,
		},
		{
			depth: "20 cm",
			temp: current?.soil_temp_20_f,
			moisture: current?.soil_moisture_20,
		},
		{
			depth: "50 cm",
			temp: current?.soil_temp_50_f,
			moisture: current?.soil_moisture_50,
		},
		{
			depth: "100 cm",
			temp: current?.soil_temp_100_f,
			moisture: current?.soil_moisture_100,
		},
	];
		...
		<div>
			...
			{data?.last_updated && (
				<p style={styles.updated}>
					...
					{data.soil_temp_trend &&
						` | Soil temp trend: ${data.soil_temp_trend}`}
				</p>
			)}
			...
			<div style={sharedStyles.gaugeGrid}>
				<div style={sharedStyles.card}>
					<Gauge {...SOIL_TEMP_GAUGE} value={current?.soil_temp_10_f ?? null} />
				</div>
				<div style={sharedStyles.card}>
					<Gauge
						{...AMBIENT_TEMP_GAUGE}
			...
			<div style={styles.summaryGrid}>
				...
				<SummaryCard
					...
					value={
						data?.precipitation_7day_total_mm != null
							? mmToInches(data.precipitation_7day_total_mm)
							: null
					}
					unit="in"
					decimals={2}
						...
						{data.forecast.daily_summary.map((day) => (
							<div key={day.date} style={styles.forecastCard}>
								...
								<div style={styles.forecastTemp}>
									{day.high_temp_f.toFixed(0)}
									{"\u00B0"} / {day.low_temp_f.toFixed(0)}
									{"\u00B0"}
								</div>
				...
				<>
					...
					{soilForecast.threshold_crossings.length > 0 && (
						<div style={styles.crossingGrid}>
							{soilForecast.threshold_crossings.map((c) => (
								<div
									key={`${c.threshold_name}-${c.direction}-${c.threshold_temp_f}`}
									style={styles.crossingCard}
								>
									...
									<div style={styles.crossingDate}>
										~{c.days_until_crossing} days (
										{new Date(
											`${c.estimated_crossing_date}T00:00:00`,
					...
					<div style={styles.modelInfo}>
						Model: R\u00B2={soilForecast.model_info.r_squared.toFixed(2)}, lag=
						{soilForecast.model_info.lag_days}d, quality=
						{soilForecast.model_info.quality}
					</div>
				</>
			)}
