...
export default function Calendar() {
	...
	const followUpsByDate = useMemo(() => {
		...
		for (const apps of Object.values(data.days)) {
			for (const app of apps) {
				...
				seen.add(key);
				if (!map[app.follow_up_date]) {
					map[app.follow_up_date] = [];
				}
	...
	const selectedApps: Application[] = selectedDate
		? (loggedAppsByDate[selectedDate] ?? [])
		: [];
	const selectedFollowUps: Application[] = selectedDate
		...
		<div>
			...
			<div style={styles.legend}>
				...
				<span style={styles.legendItem}>
					<span
						style={{
							...styles.legendBar,
							backgroundColor: ACTIVITY_STATUS_COLORS.Active,
						}}
					/>
					Active
				</span>
				<span style={styles.legendItem}>
					<span
						style={{
							...styles.legendBar,
							backgroundColor: ACTIVITY_STATUS_COLORS.Upcoming,
						}}
					/>
					Upcoming
				</span>
				<span style={styles.legendItem}>
					<span
						style={{
							...styles.legendBar,
							backgroundColor: ACTIVITY_STATUS_COLORS.Completed,
						}}
					/>
					Completed
				</span>
				<span style={styles.legendItem}>
					<span
						style={{
							...styles.legendBar,
							backgroundColor: ACTIVITY_STATUS_COLORS.Missed,
						}}
					/>
					Missed
				</span>
			...
			<div style={styles.calGrid}>
				<table style={styles.table}>
					...
					<tbody>
						{weeks.map((wk) => (
							<tr key={`week-${wk.find((d) => d !== null)}`}>
								{wk.map((day, di) => {
									if (day === null)
										return (
											<td
												key={`blank-${WEEKDAYS[di]}`}
										...
										<td
											...
											onKeyDown={(e) => {
												if (e.key === "Enter" || e.key === " ") {
													...
													if (hasContent) setSelectedDate(dk);
												}
											}}
										>
						...
						<div>
							<h4 style={styles.sectionLabel}>
								{selectedActivities.some(
									(a) => a.category === "Plant Maintenance",
								)
									? "Turf Activities"
									: "Planned Activities"}
							</h4>
									...
									<div key={activity.id} style={styles.activityDetailCard}>
										<div style={styles.activityDetailHeader}>
											<span
												style={{
													...styles.statusBadge,
													backgroundColor: `${ACTIVITY_STATUS_COLORS[activity.status]}22`,
