...
export default function NitrogenBudgetWidget({
	data,
}: NitrogenBudgetWidgetProps) {
	const {
		...
		<div style={styles.card}>
			<div style={styles.header}>
				...
				<span style={{ ...styles.rangeBadge, color: barColor }}>
					{grass_type_target.min_lbs_per_1000sqft}-
					{grass_type_target.max_lbs_per_1000sqft} lbs/1k
				</span>
			</div>

			<div style={styles.valueRow}>
				<span style={styles.bigValue}>
					{applied_lbs_per_1000sqft.toFixed(1)}
				</span>
			...
			<div style={styles.remainingRow}>
				{percent_of_target > 100 ? (
					...
					>
						Over budget by{" "}
						{(applied_lbs_per_1000sqft - target_lbs_per_1000sqft).toFixed(1)}{" "}
						lbs
					</span>
				) : (
			...
			{applications.length > 0 && (
				<div style={styles.appList}>
					{applications.map((app) => (
						<div
							key={`${app.date}-${app.product_name}-${app.n_lbs_per_1000sqft}`}
							style={styles.appRow}
						>
							<span style={styles.appDate}>{app.date}</span>
							<span style={styles.appProduct}>
								{app.product_name || "Unknown"}
							</span>
							<span style={styles.appN}>
								+{app.n_lbs_per_1000sqft.toFixed(2)}
							</span>
						</div>
					))}
				</div>
			...
			{applications.length === 0 && (
				<div style={styles.noApps}>
					No nitrogen applications recorded yet. Add N-P-K data when logging
					applications.
				</div>
			)}
		</div>
