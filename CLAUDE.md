...
- `cd backend && cargo test` — Run tests (301 tests)
...
│       ├── mcp/                 # Read-only MCP server at /mcp (rmcp): config.rs MCP_TOKEN, auth.rs bearer guard, instructions.rs doctrine, trim.rs, tools/ calling the api handlers
