...
- `cd backend && cargo test` — Run tests (314 tests)
...
│       ├── mcp/                 # Read-only MCP server at /mcp (rmcp): config.rs MCP_TOKEN, auth.rs bearer guard, instructions.rs doctrine, resources.rs (embeds docs/agronomy-methods.md + docs/timing-windows.md), trim.rs, tools/ (lawn · season · weather · disease · log · shelf) calling the api handlers
│       ├── api/                 # Route handlers (30 paths / 42 routes) + recommendation_feed.rs shared by dashboard and feed
...
│       ├── logic/               # Data sync + 10 agronomic rules + GDD accumulation + seasonal plan + disease risk models + seeding/pre-emergent timing windows + inventory.rs (shelf enrichment pass) + inventory/shopping.rs (shopping list)
...
| GET | /api/v1/inventory/shopping-list | Buy (unmet needs of active recs) + Restock (Low/Out products an active rec calls for), sorted by urgency; re-evaluates the feed |
...
| POST | /mcp | MCP server (streamable HTTP, stateless JSON, read-only tools); mounted only when `MCP_TOKEN` is set, `Authorization: Bearer` required |
...
- Product inventory (`models/product/`, `api/products.rs`, `/inventory`; plan in `docs/plans/product-inventory.md`): what is on the shelf, by `ProductCategory` (Fertilizer · Supplement · Fungicide · Herbicide · InsectControl · Seed · SoilAmendment · Surfactant · Other). **No quantities by design** — `stock_status` is a hand-set In stock / Low / Out (Out = rebuy; `archived` = no longer used). A product has user-owned **facts** (category, form, NPK, FRAC classes, herbicide timing, `ProductTarget` list, amendment kind, optional label rate) flattened into the JSON, plus an optional LLM **profile** (JSONB, `profile_model` / `profile_generated_at`) that is informational and regenerable. On add, the assistant fills the facts once (curated `frac_classes_for_product` beats the LLM's FRAC list; the user's category/form beat its guess), then `ProductFacts::scoped_to_category` drops what cannot apply — FRAC classes only on Fungicide, timing only on Herbicide, amendment kind only on SoilAmendment, targets only from the family that fits (`ProductTarget::fits_category`), `Other` keeps everything — on add, refresh and PUT alike (the LLM tags herbicides with FRAC classes and fungicides with nutrient targets); PUT edits facts only; refresh stores the new profile and returns a diff for the user to apply. The LLM schema has no interval or timing fields; its one rate is `suggested_label_rate`, stored as the product's label rate, shown with a "verify against your label" caveat, and used only to pre-fill the application form — never in advice text. A product saves without OpenRouter (unlike plants). Every enum's variant name is its serde name, DB string and JSON-schema value (`models/product/enums.rs` `string_enum!`). **Applications link to the shelf** via nullable `applications.product_id` (`ON DELETE SET NULL`); `product_name` is copied from the product at log time so history survives a rename/delete and the duplicate key is unchanged. `ProductCategory::accepts(ApplicationType)` (mirrored by `types/inventoryCompat.ts` `categoriesFor`) gates which products the form offers and which the API accepts; `Other` on either side accepts anything. Picking a product pre-fills name, N-P-K, FRAC classes (the `FracClassPicker` is remounted with `recorded` so the name lookup does not overwrite them) and the label rate; logging against an `Out` product offers to mark it In stock (the only stock automation). The Inventory page lists logged-but-uncatalogued names (`LogSuggestions`) with one-click add (name + guessed category) and each product has "Link past applications".
- Inventory in recommendations (`models/inventory.rs`, `logic/inventory.rs`): a source declares what its action calls for with `Recommendation::with_need(ProductNeed)` — categories / FRAC classes / herbicide timing / targets / amendment kinds, each any-of, plus `optional` for nice-to-haves (the irrigation rule's wetting agent) that never make a rec "need to buy". Declared by grub control (InsectControl · Grubs), broadleaf (Herbicide · PostEmergent · Broadleaf), timing pre-em windows (Herbicide · PreEmergent) and seeding windows (Seed · the lawn's species; Mixed = any cool-season), the four feeding recs (Fertilizer; `nitrogen_guard` clears the need at "target reached"), soil-test pH / NPK / micronutrients (lime or sulfur kinds, fertilizer, per-nutrient Supplement or gypsum / dolomitic lime), plant Fertilizing tasks (Fertilizer or Supplement) and disease alerts (Fungicide in the program's unrestricted, not-last-used classes; red thread → Fertilizer only while nitrogen-hungry, i.e. no fertilizer logged in 60 d — once fed the advice is "stay the course" and nothing is needed). `enrich()` runs in `recommendation_feed::all` after every source and **before** persisted answers (ids and severities untouched): `InventoryStatus::assess` matches usable products (not archived, not Out) **strictly** — a product with no class / timing / targets recorded does not satisfy a need that asks for one — and sets `rec.inventory` (OnHand · Low = every match Low · NotOnHand · Partial) plus "On hand" / "Not on shelf" data points. Disease management sees the shelf via `DiseaseContext.shelf` (usable fungicides, filled in `api/disease_risk.rs`): `recommended_class` prefers an owned class **only among equally efficacious eligible options** — never over efficacy or rotation — `FungicideOption.on_hand` lists the products and the headline says "You have X on hand" / "Nothing on your shelf is in FRAC n". UI: `InventoryBadge` on dashboard `AlertCard`s and the Recommendations list, an "Inventory" section in the detail pane, and an "On shelf" tag on `ManagementPanel` options.
- Shopping list (`logic/inventory/shopping.rs`, `GET /inventory/shopping-list`): built from the **active feed only** — every future use is already an active rec (timing windows enter the feed as Info while OpeningSoon), so there is no separate seasonal-plan lookahead (its activity categories are prose strings anyway). `Buy` = a required need in `rec.inventory.missing`; `Restock` = a non-archived Low/Out product that `need.fits()` (facts only, stock ignored). A restockable product **is** the thing to buy, so it suppresses the generic Buy line for that need. Answered recs and `optional` needs never shop. Sorted by urgency (max severity of the reasons), then Buy before Restock. Rendered by `ShoppingListPanel` on the Inventory page (keyed on a shelf version so it re-fetches after any stock change) and compact on the dashboard (hidden when empty; top 4 + "n more").
...
- MCP server (`mcp/`, plan in `docs/plans/ai-assistant.md`): rmcp streamable HTTP merged into the Axum app **after** the CORS layer, stateless + JSON responses, rmcp's loopback-only `Host` check disabled (the gateway calls by LAN hostname; the bearer is the guard). Tools call the REST handlers directly (`tools::call` / `call_trimmed`), so answers match the UI; results are JSON text with bulk trimmed in `trim.rs` (hourly forecast, chart series, disease detail). `lawn_snapshot` composes profile + conditions + the **full** feed (the dashboard truncates to 3) + 5 recent applications + timing + disease headlines + N totals; a failed part is `null` with a note. Handler errors become tool-level errors (`isError`) with model-readable text; lake / upstream / internal details stay in the log. Every tool is `read_only_hint`; each `tools/*.rs` is its own `#[tool_router(router = …)]` summed in `TurfOpsMcp::tools`. `weather_history` does **not** use `/historical` (which downsamples hourly points for charts, so its rain can't be summed); it joins the lake's per-local-day silver aggregates (`fetch_daily_disease_inputs`) with the memoized climate record (`api::timing::climate_record`). Resources are `include_str!` of `docs/` — the Dockerfile copies those two files to `/docs` in the build stage so the relative path resolves; `docs/agronomy-methods.md` must be kept in sync with thresholds in code.
...
- `MCP_TOKEN` — bearer token for `/mcp` (≥ 32 chars; unset = not mounted)
